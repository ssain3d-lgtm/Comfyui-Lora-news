...
class ServiceTests(unittest.TestCase):
    def test_first_run_baseline_then_new_detection(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            svc = make_service(tmp, lambda: (hf[:-1], []), lambda: (gh, []))
            counts = svc.refresh()
            ...
            svc2 = make_service(tmp, lambda: (hf, []), lambda: (gh, []))
    ...
    def test_source_failure_keeps_previous_cache(self):
        ...
        gh = [it for it in items if it.source == "github"]
        with tempfile.TemporaryDirectory() as tmp:
            make_service(tmp, lambda: (hf, []), lambda: (gh, [])).refresh()

            def boom():
                raise RuntimeError("network down")

            svc = make_service(tmp, boom, lambda: (gh, []))
            counts = svc.refresh()
            self.assertEqual(counts["huggingface"], len(hf))
            self.assertTrue(any("HuggingFace 수집 실패" in e for e in svc.status["errors"]))
            self.assertTrue(any("이전 캐시" in e for e in svc.status["errors"]))
    ...
    def test_claude_summaries_applied_and_kept(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("base_models", snap["facets"])
